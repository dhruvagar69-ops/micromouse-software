...
void Agent::update(const IndexVec &cur, const Direction &cur_wall)
{
	...
	if (state == Agent::SEARCHING_NOT_GOAL) {
		...
		if (distIndexList.empty()) {
			...
			//put the element clossest to the current coordinate from distIndexList into dist
	...
	if (state == Agent::SEARCHING_REACHED_GOAL) {
		//Update if any element in distIndexList is reached or if the destination is determined to be unreachable
		auto it = std::find(distIndexList.begin(), distIndexList.end(), cur);
		if (it != distIndexList.end() || calcNextDirection(cur, dist) == 0) {
			// list one by one the coordinates of unexplored walls along the provisional shortest path
            // Add those coordinates to distIndexList
			distIndexList.clear();
	...
	if (state == Agent::BACK_TO_START) {
		if (toDistinationPath.empty()) {
			// Calculate the shortest route from the current location to the starting point
...
void Agent::resumeAt(State resumeState, Maze &_maze)
{
	...
	*maze = _maze;
	...
	if (resumeState == State::IDLE) {
		...
		// Enumerate the coordinates of unexplored walls along the provisional shortest path
        // Add those coordinates to distIndexList
		...
		// Set the closest element in `distIndexList` to the current coordinates to `dist`
		int minDistance = INT32_MAX;
		std::list<IndexVec>::iterator it_nearestDist;
		for (auto it=distIndexList.begin();it!=distIndexList.end();it++) {
