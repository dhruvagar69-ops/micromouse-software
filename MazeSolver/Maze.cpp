...
void Maze::updateWall(const IndexVec &cur, const Direction& newState, bool forceSetDone)
{
	...
	// reflect the current EAST to WEST at position x+1
    // reflect the current NORTH to SOUTH at position y+1
    // reflecting the current WEST to EAST at position x-1
    // reflecting the current SOUTH to NORTH at position y-1
	for (int i=0;i<4;i++) {
		if (cur.canSum(IndexVec::vecDir[i])) {
			IndexVec neighbor(cur + IndexVec::vecDir[i]);
			//Reflect the current i-th wall information bit and the Done bit to the (i+2)%4-th position (rotated 180 degrees)
	...
	while (!q.empty()) {
		...
		for (int i=0;i<4;i++) {
			...
			if (!cur_wall[i] && stepMap[scanIndex.y][scanIndex.x] > stepMap[cur.y][cur.x] +1) {
				//What to do with unexplored walls
